...
Route::middleware(['passed.entrance'])->group(function () {
    ...
    Route::middleware(['block.staff.manager'])->group(function () {
...
Route::group(['prefix' => 'seller', 'middleware' => ['auth'], 'as' => 'seller.', 'namespace' => 'App\Http\Controllers\Seller'], function () {
    ...
    Route::get('/seller/invites', [InviteController::class, 'index'])->name('invite.index');
    Route::get('/seller/invites/create', [InviteController::class, 'create'])->name('invite.create');
    Route::post('/seller/invites', [InviteController::class, 'store'])->name('invite.store');

    Route::get('/seller/invites/logs', [InviteController::class, 'logs'])->name('invite.logs');

    Route::get('/invite/reinvite/{id}', [InviteController::class, 'reInvite'])->name('invite.reinvite');
    Route::get('/invite/expire/{id}', [InviteController::class, 'expire'])->name('invite.expire');

    Route::get('/staff/register', [StaffRegiController::class, 'showForm'])->name('staff_regi.show');
    Route::post('/staff/register', [StaffRegiController::class, 'register'])->name('staff_regi_logs.show');
...
// 管理画面　❗「Voyagerは“なんでもルート”を持っている」👉 だから必ず最後に置く
Route::group([
    'prefix' => 'admin',
